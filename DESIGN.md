...
### Primary
- **Electric Iris** (#6c63ff): The journal's ink for everything alive — links in their active state, the typewriter role line in the hero, eyebrow labels, active nav items, badge borders, and the primary button gradient. It is the color of momentum and appears only where attention should land.
- **Iris Mist** (#eeeeff): Electric Iris diluted to a wash. Backgrounds for active nav pills, tech badges (at 75% opacity via color-mix), and the hero gradient's start. Never used for text.
...
- **Slate Muted** (#6b7280): Paragraph text and inactive nav links. Cool gray, used at 1rem+ on white where it holds ~4.7:1 contrast — do not use it smaller or on tinted surfaces.
...
- **Label** (JetBrains Mono, 0.78rem, letter-spacing 0.16em, uppercase): Eyebrows and section numbers, in Electric Iris or Slate Muted.
...
- **Do** use JetBrains Mono for all metadata voice — eyebrows, section numbers, badges — at 0.78–0.84rem.
...
- **Don't** ship dark-theme UI until real `[data-theme='dark']` token overrides exist; the toggle without the tokens is a broken promise.
