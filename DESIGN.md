...
An ambient WebGL black hole behind the hero's right side (~70% of the section): a tilted accretion disk of instanced light streaks orbiting an event horizon, recolored from the classic orange into the Electric Iris ramp (deep indigo rim → iris body → near-white core) so the one-accent system holds, feathered into the paper by a masked deep-space radial so it reads as background, never a panel. The copy always sits on plain paper (`z-index` above; the scene tucks into the bottom-right corner on small screens at reduced opacity). This is the site's one deliberate exception to the no-WebGL stance, and it stays quiet: no controls, no HUD, no interaction. Performance is part of the design — three.js lives in its own lazy chunk loaded during idle time after first render (the initial bundle never sees it), the loop pauses off-screen and on hidden tabs, pixel ratio is capped (2 desktop / 1.5 mobile), particle count drops on small screens (4500 → 2600), and reduced motion gets a single still frame. The tilt is one configurable knob (`tiltDeg`, default 25°) on `app-black-hole`.
