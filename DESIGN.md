...
A glass jar of fireflies beside the skills list — each firefly is one skill from `PORTFOLIO.skills.jar.items` (max 9; the scene defines 9 spots). The jar is drawn in CSS on the same dusk-ink panel language as the Night Garden: moonlight glass borders, a metal lid in the scene ramp, light pooled at the bottom, faint stars behind. Fireflies blink and drift idly, light up one by one when the panel scrolls into view, and on hover/focus flare brighter and name themselves in a mono pill (keyboard-reachable via `tabindex` + `aria-label`; the full skill list remains as real text in the right column, so the jar is enhancement, not the record). Caption from `PORTFOLIO.skills.jar.caption`. Same illustration exemptions as the Night Garden: scene geometry and scene-ramp colors are drawing, not chrome. The narrative pairing is deliberate — the jar (Skills) is where the fireflies are kept; the garden (Beyond) is where they fly free.
