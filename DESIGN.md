...
### Hero Scene: "The Singularity" (signature component)

An ambient WebGL black hole behind the hero's right side (~70% of the section): a tilted accretion disk of instanced ink streaks orbiting an event horizon, drawn straight on the paper — no dark backdrop, normal alpha blending — so it reads like line-work, never a panel. The color ramp runs sun-white and pale gold at the core (a warm rim glow hugs the horizon) out through Electric Iris to deep indigo at the edge; a radial mask feathers every boundary. The copy always sits on plain paper (`z-index` above on desktop). **Small screens (<768px) run the scene level — no tilt — and edge to edge:** on tablets it spans the bottom half of the hero behind clear space; on phones (≤640px) it leaves the background entirely and becomes its own full-bleed band below the copy (the hero grows past one screen), so the whole disk shows and never sits behind text. This is the site's one deliberate exception to the no-WebGL stance, and it stays quiet: slow orbit, no controls, no HUD, no interaction. Performance is part of the design — three.js lives in its own lazy chunk loaded during idle time after first render (the initial bundle never sees it), the loop pauses off-screen and on hidden tabs, pixel ratio is capped (2 desktop / 1.5 mobile), particle count drops on small screens (3600 → 2000), and reduced motion gets a single still frame. The tilt is configurable per register (`tiltDeg`, default 25°; `tiltDegSmall`, default 0° below 768px) on `app-black-hole`.

### About Scene: "The Ascent" (signature component)

The code-drawn SVG staircase of version blocks — v8 → v14 → v17 rising left to right, a builder figure on the top step reaching toward the floating iris v19 block above its dash-outlined landing slot, sparks around it, clouds below (we are at altitude). It is his actual migration ladder drawn as a scene: energy and ascent, not decoration. It anchors the About section's left column beside the story it illustrates (below the copy on small screens). Drawn entirely from the brand palette and the iris scene ramp — no raster assets, nothing to 404. Reduced motion freezes every layer. The block labels come from `PORTFOLIO.identity.ascent`.
...
The journey as an ink-drawn mountain panorama stretching the full section width — every peak is a chapter, drawn taller as the career climbs (college-lab foothill on the left up to the current role's summit on the right). Generated at runtime from `PORTFOLIO.journey.entries`: each mountain gets a seeded, deterministic silhouette (offset summit, uneven flanks, occasional secondary peak) filled with 8–14 ridge echoes — the whole ridge profile repeated downward at an even step with a slow hand-drawn sine wobble, some trailing off down one favoured flank, all clipped to the silhouette so no line ever leaks outside — so no two mountains match and adding a chapter adds a mountain. Each summit carries a minimal marker — dot, leader line, start year, `peakLabel` (defaults to `place`) — and the whole mountain body is a hover/click target alongside the marker button; selecting a chapter: its mountain tints Iris Mist with iris strokes, and the full journal card (story, bullets, highlight, stack) swaps in below the panorama, one at a time, never covering a peak. All cards stay in the DOM (`[hidden]` toggling) for prerender/SEO, and print shows every chapter. Long chapters preview two bullets with a quiet "+N more / Show less" text toggle (per-chapter state, `aria-expanded`/`aria-controls`); below 860px the stack row also previews four badges behind a dashed "+N more" pill (desktop shows the full stack — no space constraint there). Mountains rise staggered on scroll-into-view; reduced motion renders them in place. **On phones (≤860px, `CAROUSEL_QUERY`) the section reshapes around touch:** the panorama keeps a 680px canvas inside a pannable strip (sky padding inside the scroller so summit labels never clip), and the cards become a swipeable scroll-snap deck with the neighbour peeking in as the swipe affordance — equal-height slides so cycling chapters never scrolls the page vertically. The two strips stay in sync both ways: swiping a card lights up its peak and pans the panorama to it; tapping a peak slides its card in horizontally. Both open at their right end on the newest chapter. This is the daylight scene — ink line-work on Cloud Surface paper with a mono corner caption from `PORTFOLIO.journey.rangeCaption` (desktop only) — the section-scale echo of the Ascent's altitude metaphor.
...
An ink dusk panel beside the Beyond copy: blooming iris flowers with layered 3D-tilted petals, gradient stems that grow on scroll-into-view, rising light motes, silhouetted foliage, faint stars, and a dashed moon ring echoing the Ascent's ring — the after-hours counterpart to the Ascent's daytime altitude. Built entirely from HTML divs and CSS (organic border-radius petals, gradients, keyframes — the CodePen flowers technique) in the scene ramp; no images, no SVG, no canvas, no WebGL, no libraries. Sized in container-query units so the scene scales with its panel; per-flower scale via an `--u` em base. The grow/bloom entrance plays only when the panel scrolls into view (`.is-visible`); the settled scene is the natural state for no-JS, print, and reduced motion, where ambient sway/blink/motes also freeze. **Scene geometry is drawing, not chrome:** its border-radius values (petal blobs like `51% 49% 47% 53%`, `1em`/`4em`–`10em` shape corners) are illustration shapes exempt from the interface radius scale, exactly as scene-ramp colors are exempt from the interface palette. The mono caption comes from `PORTFOLIO.beyond.sceneCaption`.

### Project Covers

Project cards show a real screenshot when the config provides one (`image` in `ProjectItem`, served from `/public/images`, `object-fit: cover` anchored to the top edge so headers never crop away). Cards without an image draw their own cover art: a `--cover-hue`-rotated iris gradient wash with an oversized mono monogram — the wash also doubles as the loading backdrop behind lazy screenshots. Hue rotation stays subtle (±40° around the brand hue). The config spec guards that every referenced image file exists.
