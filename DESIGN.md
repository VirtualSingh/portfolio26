...
A glass mason jar of skills disguised as fireflies beside the skills list — each is one entry from `PORTFOLIO.skills.jar.items` (max 9, `{ label, icon }`; the scene defines 9 spots and the icon set lives in `JarSkillIcon`). The jar is drawn in CSS on the same dusk-ink panel language as the Night Garden: an open ribbed screw mouth and collar (no lid), moonlight glass borders and highlight streaks, light pooled at the bottom, faint stars behind. Inside float ghostly stroke-SVG skill glyphs (Angular shield, TS mark, RxJS marble stream, Sass S, Jest check, Playwright mask, git branch, `</>`, Tailwind waves) glowing via layered drop-shadows — the icons-as-fireflies conceit from the inspiration, rebuilt as code instead of a PNG. They blink and drift idly, light up one by one when the panel scrolls into view, and on hover/focus flare brighter and name themselves in a mono pill (keyboard-reachable via `tabindex` + `aria-label`; the full skill list remains as real text in the right column, so the jar is enhancement, not the record). Caption from `PORTFOLIO.skills.jar.caption`. Same illustration exemptions as the Night Garden: scene geometry and scene-ramp colors are drawing, not chrome. The narrative pairing is deliberate — the jar (Skills) is where the fireflies are kept; the garden (Beyond) is where they fly free.
