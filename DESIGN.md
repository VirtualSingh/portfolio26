...
An ambient WebGL black hole behind the hero's right side (~70% of the section): a tilted accretion disk of instanced ink streaks orbiting an event horizon, drawn straight on the paper — no dark backdrop, normal alpha blending — so it reads like line-work, never a panel. The color ramp runs sun-white and pale gold at the core (a warm rim glow hugs the horizon) out through Electric Iris to deep indigo at the edge; a radial mask feathers every boundary. The copy always sits on plain paper (`z-index` above; the scene tucks into the bottom-right corner on small screens at reduced opacity). This is the site's one deliberate exception to the no-WebGL stance, and it stays quiet: slow orbit, no controls, no HUD, no interaction. Performance is part of the design — three.js lives in its own lazy chunk loaded during idle time after first render (the initial bundle never sees it), the loop pauses off-screen and on hidden tabs, pixel ratio is capped (2 desktop / 1.5 mobile), particle count drops on small screens (3600 → 2000), and reduced motion gets a single still frame. The tilt is one configurable knob (`tiltDeg`, default 25°) on `app-black-hole`.
